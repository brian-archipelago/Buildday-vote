...
export async function POST(req: NextRequest) {
  ...
  if (kind === "segment") {
    ...
    setState((s) => {
      ...
      // Keep the most recent committed text visible between utterances so the
      // operator sees a continuous feed instead of blanking out during pauses.
      // The next utterance's first delta will overwrite this.
      s.liveTranscript = text;
    });
    ensureAnalyzerTimer();
...
export async function DELETE(req: NextRequest) {
  ...
  setState((s) => {
    ...
    // Fresh start clears the deleted-title tombstones too -- there's no
    // transcript left for them to protect against.
    s._deletedTitles = [];
  });
  return Response.json({ ok: true });
}
